 ...
 * choice together, so the two can never disagree. Every surface that switches
 * workspace used to do both halves itself — the picker, the mode toggle, the
 * setup wizard — and each did a slightly different subset, which is how a
 * switch could leave the store on one workspace and the cookies on another.
 *
